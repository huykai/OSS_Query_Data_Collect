...
class QueryOptions(object):
    def __init__(self):
        ...
        self.selectunittype = 'MME'         # 查询的统计粒度（'MME/MME#TA_ID/MME#UNIT_ID/SAEGW/SAEGW#SSPROF_ID/...')
    ...
    def getQueryoptions(self,info):
        ...
        try:
            opts, args = getopt.getopt(info[1:], "e:c:D:T:d:t:p:u:l:R:hH") 
        except Exception, e:
            result = 1
            return result
        lenofargs=len(opts)
...
def printusage(UsageString,info):
    print UsageString
    print "Usage:	" + info[0] + " -D2015/04/03 -T0400 -d2015/04/03 -t0500 -eSHMME03BNK -cUNIT_ETH_INOCTETS,UNIT_ETH_OUTOCTETS,UNIT_ETH_TXFRAMES,UNIT_ETH_RXFRAMES -p60 -uMME#UNIT_ID,LIF_ID -l1 -RReport_20160701.json"
    ...
    print "-u (Unittype     :Type MME/MME:TA_ID/MME:UNIT_ID/SAEGW/SAEGW:SSPROF_ID/MME:UNIT_ID,LIF_ID/...)"
...
def getStatSQLstring(table_name,column_list,queryoptions):
    ...
    # Generate Select String
    SelectString = makequery_selectpart(queryoptions)
    # add some key (other than the NE and the period_start_time)
    SelectString = SelectString + makequery_otherkeys(queryoptions.selectunittype)
    ...
    WhereString =  makequery_where_unittype(queryoptions.selectunittype)
    ...
    GroupString = makequery_groupstring(queryoptions)
...
def makequery_selectpart(queryoptions):
    SelectString = '''
select
objects.CO_NAME,
...
def makequery_groupstring(queryoptions):
    # some query need other keys in sql ,like lif_id ,so the selectunittype can be string
    # like "MME:UNIT_ID,LIF_ID" , MME mean NE type, UNIT_ID,LIF_ID mean unit type
    querystring_grouppart = ''
    keystring = queryoptions.selectunittype
    keypair = keystring.split('#')
    unit_type = ''

    # Normal we use "MME" or "MME:UNIT_ID,LIF_ID" and the default value is "MME" also
    if (len(keypair)<=1):
        querystring_grouppart = "MME"
    else:
        #keypair like ['MME','UNIT_ID,LIF_ID']
        ne_type = keypair[0]
        unit_type = keypair[1]
...
"""
    ...
    return GroupString + orderstring

def makequery_otherkeys(selectunittype):
    # some query need other keys in sql ,like lif_id ,so the selectunittype can be string
    # like "MME:UNIT_ID,LIF_ID" , MME mean NE type, UNIT_ID,LIF_ID mean unit type
    querystring_keypart = ''
    keystring = selectunittype
    keypair = keystring.split('#')

    # Normal we use "MME" or "MME:UNIT_ID,LIF_ID" and the default value is "MME" also
    if (len(keypair)<=1 or (len(keypair)>1 and keypair[1] == '')):
        querystring_keypart = "'ALL' as ElementType "
    else:
        #keypair like ['MME','UNIT_ID,LIF_ID']
        ne_type = keypair[0]
        unit_type = keypair[1]
        querystring_keypart = " " + unit_type + " "
    return querystring_keypart

def makequery_where_unittype(unittype):
    # some query need other keys in sql ,like lif_id ,so the selectunittype can be string
    # like "MME:UNIT_ID,LIF_ID" , MME mean NE type, UNIT_ID,LIF_ID mean unit type
    querystring_keypart_where = ''
    keystring = unittype
    keypair = keystring.split('#')
...
"""
    return querystring_keypart_where
...
def GenerateReportFile(rowinfo , queryoptions):
    ...
    if (queryoptions.selectcounters.find('ALARM')<0):
        reportStaTitle(queryoptions,reportfile)
    else:
        reportAlatmTitle(queryoptions,reportfile)
...
def reportStaTitle(queryoptions,reportfile):
    reportfile.write('{\n')
    ...
    statetype = queryoptions.selectunittype.split('#')
    if (len(statetype)<=1 or (len(statetype)>1 and statetype[1]=='')):
        reportfile.write(',\n\t\t"StatType"')
    else:
        unit_types = statetype[1]
...
def reportAlarmTitle(queryoptions,reportfile):
    reportfile.write('{\n')
    reportfile.write('\t"columndesc": [\n')
    reportfile.write('\t\t"NE_NAME",\n')
    reportfile.write('\t\t"DN",\n')
